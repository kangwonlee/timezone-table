...
def test_write_xl_table_header_and_date(xl_table_fixture):
    ...
    # Date row — per-city local dates (Sydney is ahead of the dateline)
    assert ws["A2"].value == "Date"
    assert ws["B2"].value == "2026-01-15"  # LA same date
    assert ws["C2"].value == "2026-01-16"  # Sydney is next day
...
def test_write_xl_table_dateline_dates(tmp_path):
    ...
    assert ws["C2"].value == "2026-01-16"  # Sydney: next day
